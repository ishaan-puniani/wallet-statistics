...
const TransactionRuleValidation = (props: ITransactionRuleValidation) => {
  ...
  useEffect(() => {
    const fetchTransactionData = async () => {
      ...
      const fetchTransactionData = await axios.post(
        `${props.credentials.API_HOST || API_HOST}/tenant/${props.credentials.application_id}/get-transaction-type-autocomplete`,
        {
          ...props.credentials,
        }
      );
  ...
  const onSubmit = async (data: any) => {
    const validate = await axios.post(
      `${props.credentials.API_HOST || API_HOST}/tenant/${props.credentials.application_id}/validate-transaction-rule`,
      {
        data,
      }
    );
